...
// appID Abfrage (Prompt)
var appID = prompt("Add your App ID");
...
window.fbAsyncInit = function () {
  ...
  FB.login(function (response) {
    ...
    if (response.authResponse) {
      ...
      console.log("Welcome! You have been logged in successfully.");
    ...
    // wenn das Login fehlschlägt
      ...
      console.log("User canceled login or is not fully authorized.");
    }
  });
  ...
  const getData = async (actid) => {
    ...
    const response = await fetch(
      `https://graph.facebook.com/v9.0/${actid}/insights?fields=impressions,clicks,spend,account_currency&access_token=${accessToken}`
    );
  ...
  const updateData = async (filterVal) => {
    FB.api(
      ...
      (response) => {
        const { spend, impressions, clicks, account_currency } =
          response.data[0];
        // Hinzufügen von Daten zur index Datei + Umformatierung dieser
        $("#amount").text(
          spend.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        );
        $("#impression").text(
          impressions.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        );
        $("#clicks").text(
